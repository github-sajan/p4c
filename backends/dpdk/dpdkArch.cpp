...
/* This function processes each header structure and align each non-aligned header
   into 8-bit aligned header.[Restricted to only continuos non-aligned header fields]
   For example :
       header ipv4_t {
            bit<4>  version;
            bit<4>  ihl;
...
const IR::Node *AlignHdrMetaField::preorder(IR::Type_StructLike *st) {
    ...
    if (st->is<IR::Type_Header>()) {
            ...
            if ((*field).type->is<IR::Type_Bits>()) {
                ...
                if (width % 8 != 0) {
                    ...
                    if (size_sum_so_far != 0) {
                        ::error("8-bit Alignment for Header Structure '%1%' is not possible",
                                st->name.name);
                        return st;
                    }
        ...
        /* Throw error if there is non-aligned field present in header */
        if (size_sum_so_far != 0) {
...
/* This function process the occurance of non-aligned fields and replaces it with
   modified member with proper lsb and msb as a slice Member 
